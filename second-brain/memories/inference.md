...
This document outlines the detailed architecture of the Hybrid Fused Volumetric MRI Classifier (S4 + 3D CNN) and specifies the exact inner workings of the C++ inference engine using ONNX Runtime.

---

## 1. Model Architecture & Parameters

The model is designed as a hybrid dual-branch architecture for patient-level volumetric K-space slice classification. It processes raw multi-slice complex K-space data through parallel Frequency (S4 SSM) and Spatial (3D CNN) branches, fusing their outputs using Slice-Level Cross-Attention.

```
                  Raw Volumetric K-Space Input
                 [B=1, S=64, C=1, H=128, W=128]
                               |
            +------------------+------------------+
            |                                     |
            v                                     v
  ...
  (Complex Linear Encoder)            (Differentiable 2D IFFT)
            |                                     |
            v                                     v
   (2-Layer S4D Block)                   (4-Layer 3D CNN)
            |                                     |
            v                                     v
...
*   **Projection:** Flattens each K-space slice ($C \times H \times W = 16384$) and projects it to a $128$-dimensional complex latent space.
...
### 1.2 Spatial Branch (3D CNN)
*   **Purpose:** Learns global volumetric and multi-slice spatial features.
*   **Reconstruction:** Applies a differentiable 2D IFFT on the raw K-space slices, computes the root-sum-of-squares (RSS) combination across coils, and stacks them into a magnitude volume: $V_{\text{mag}} \in \mathbb{R}^{B \times 1 \times S \times H \times W}$.
*   **Convolutions:** Passes the volume through four 3D Convolution layers (with Batch Normalization and ReLU) downsampling the spatial grid.
*   **Pooling:** Applies an adaptive average pooling along the spatial dimensions to reduce them to $1 \times 1$ while mapping the depth (slice) dimension to $S=64$.
...
*   **Pooling & Head:** Adds residual connection from Query, applies global average pooling across the slice sequence ($S=64$), and projects to classification logits for 11 pathology classes.
...
*   **Real/Imaginary Representation:** Complex inputs and parameters are split into stacked real and imaginary channels. The raw K-space input shape is represented as `[B=1, S=64, C=1, H=128, W=128, 2]` where the last dimension holds `[Real, Imag]`.
...
*   **Export File:** [fused_model.onnx](file:///home/jemin/Projects/MRI/KVISION/ai-service/fused_model.onnx) (22.7 MB).

---

## 3. C++ Inference Logic

The C++ inference engine is implemented using **ONNX Runtime (ORT)** in `kvision_inference.cpp`. It loads the model, sets up execution providers, runs the forward pass, and parses the outputs.

### 3.1 Tensor Dimensions & Layouts
*   **Input Name:** `kspace_real_imag`
*   **Input Shape:** `[1, 64, 1, 128, 128, 2]` (Batch, Slices, Channels, Height, Width, Real/Imaginary)
*   **Input Type:** `float32` (flat vector of size $1 \times 64 \times 1 \times 128 \times 128 \times 2 = 2,097,152$ elements)
*   **Output Name:** `logits`
...
1.  **Engine Initialization:**
    ...
    *   Sets session options including intra-op threads (CPU execution) and graph optimizations.
